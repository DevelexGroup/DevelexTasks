...
export const sentenceWordComplete = derived(
	sentenceWord,
	($sentenceWord) => $sentenceWord >= sentences[get(sentence)].length - 1
);
...
export const zeroLevelComplete = derived(
	zeroLevelProgress,
	($zeroLevelProgress) => $zeroLevelProgress >= words.length - 1
);
