import type { LessonConfigMap } from '$lib/types/lesson';

const level1content: LessonConfigMap['pairedReading']['data']['content'] = [
	{
		text: [['dům']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0-0'
			}
		]
	},
	{
		text: [['voda']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-1-0-0'
			}
		]
	},
	{
		text: [['kolo']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-2-0-0'
			}
		]
	},
	{
		text: [['mísa']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-3-0-0'
			}
		]
	},
	{
		text: [['doba']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-4-0-0'
			}
		]
	},
	{
		text: [['ráno']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-5-0-0'
			}
		]
	},
	{
		text: [['strana']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-6-0-0'
			}
		]
	},
	{
		text: [['člověk']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-7-0-0'
			}
		]
	},
	{
		text: [['pravda']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-8-0-0'
			}
		]
	},
	{
		text: [['chvíle']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-9-0-0'
			}
		]
	}
];

const level4content: LessonConfigMap['pairedReading']['data']['content'] = [
	{
		text: [['Kočka', 'mňouká', 'a pes', 'štěká.']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			}
		]
	},
	{
		text: [['Maruška', 'právě', 'dělá', 'domácí', 'úkoly.']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 4],
					[0, 4]
				],
				id: 'fixw-0-4'
			}
		]
	},
	{
		text: [['Domácí', 'úkoly', 'Marušku', 'velmi', 'baví.']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 4],
					[0, 4]
				],
				id: 'fixw-0-4'
			}
		]
	},
	{
		text: [['Pilná', 'Maruška', 'má', 'nejraději', 'matematiku.']],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 4],
					[0, 4]
				],
				id: 'fixw-0-4'
			}
		]
	},
	{
		text: [
			[
				'Jana',
				'byla',
				'nemocná',
				'a',
				'smutně',
				'pozorovala',
				'z',
				'okna',
				'hrající',
				'si',
				'kamarády',
				'na',
				'hřišti.'
			]
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 4]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 5],
					[0, 5]
				],
				id: 'fixw-0-4'
			},
			{
				range: [
					[0, 6],
					[0, 7]
				],
				id: 'fixw-0-5'
			},
			{
				range: [
					[0, 8],
					[0, 9]
				],
				id: 'fixw-0-6'
			},
			{
				range: [
					[0, 10],
					[0, 10]
				],
				id: 'fixw-0-7'
			},
			{
				range: [
					[0, 11],
					[0, 12]
				],
				id: 'fixw-0-8'
			}
		]
	},
	{
		text: [
			[
				'Najednou',
				'Jana',
				'zpozorovala',
				'stín',
				'pohybující',
				'se',
				'tajemně',
				'mezi',
				'stromy',
				'u',
				'vedlejšího',
				'domu.'
			]
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 4],
					[0, 5]
				],
				id: 'fixw-0-4'
			},
			{
				range: [
					[0, 6],
					[0, 6]
				],
				id: 'fixw-0-5'
			},
			{
				range: [
					[0, 7],
					[0, 7]
				],
				id: 'fixw-0-6'
			},
			{
				range: [
					[0, 8],
					[0, 8]
				],
				id: 'fixw-0-7'
			},
			{
				range: [
					[0, 9],
					[0, 10]
				],
				id: 'fixw-0-8'
			},
			{
				range: [
					[0, 11],
					[0, 11]
				],
				id: 'fixw-0-9'
			}
		]
	},
	{
		text: [
			[
				'Druhý',
				'den',
				'ráno',
				'Jana',
				'uviděla',
				'tmavou',
				'a',
				'chundelatou',
				'kočku,',
				'která',
				'ji',
				'upřeně',
				'sledovala.'
			]
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 0]
				],
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 1],
					[0, 1]
				],
				id: 'fixw-0-1'
			},
			{
				range: [
					[0, 2],
					[0, 2]
				],
				id: 'fixw-0-2'
			},
			{
				range: [
					[0, 3],
					[0, 3]
				],
				id: 'fixw-0-3'
			},
			{
				range: [
					[0, 4],
					[0, 4]
				],
				id: 'fixw-0-4'
			},
			{
				range: [
					[0, 5],
					[0, 5]
				],
				id: 'fixw-0-5'
			},
			{
				range: [
					[0, 6],
					[0, 7]
				],
				id: 'fixw-0-6'
			},
			{
				range: [
					[0, 8],
					[0, 8]
				],
				id: 'fixw-0-7'
			},
			{
				range: [
					[0, 9],
					[0, 9]
				],
				id: 'fixw-0-8'
			},
			{
				range: [
					[0, 10],
					[0, 10]
				],
				id: 'fixw-0-9'
			},
			{
				range: [
					[0, 11],
					[0, 11]
				],
				id: 'fixw-0-10'
			},
			{
				range: [
					[0, 12],
					[0, 12]
				],
				id: 'fixw-0-11'
			}
		]
	}
];

const level6content: LessonConfigMap['pairedReading']['data']['content'] = [
	{
		text: [
			['Anička', 'má', 'velkou', 'rodinu.', 'Ona,', 'její', 'rodiče', 'a', 'malý'],
			['bratr', 'právě', 'slaví', 'Vánoce.', 'Všichni', 'se', 'moc', 'těší!']
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 3]
				], // First sentence
				id: 'fixw-0-0'
			},
			{
				range: [
					[0, 4],
					[1, 3]
				], // Second sentence
				id: 'fixw-0-1'
			},
			{
				range: [
					[1, 4],
					[1, 7]
				], // Third sentence
				id: 'fixw-0-2'
			}
		]
	},
	{
		text: [
			[
				'Petr',
				'šel',
				'odpoledne',
				'na',
				'zahradu.',
				'Vzal',
				'si',
				'lupu,',
				'aby',
				'se',
				'podíval,'
			],
			['co', 'tam', 'najde.', 'Nejprve', 'uviděl', 'housenku,', 'jak', 'leze', 'po', 'listu.'],
			['Potom', 'našel', 'mraveniště.', 'Mravenci', 'nosili', 'drobky', 'a', 'běhali', 'sem'],
			['a', 'tam.']
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[0, 4]
				], // First sentence
				id: 'fixw-1-0'
			},
			{
				range: [
					[0, 5],
					[1, 2]
				], // Second sentence
				id: 'fixw-1-1'
			},
			{
				range: [
					[1, 3],
					[1, 9]
				], // Third sentence
				id: 'fixw-1-2'
			},
			{
				range: [
					[2, 0],
					[2, 2]
				], // Fourth sentence
				id: 'fixw-1-3'
			},
			{
				range: [
					[2, 3],
					[3, 1]
				], // Fifth sentence
				id: 'fixw-1-4'
			}
		]
	},
	{
		text: [
			[
				'Jednoho',
				'letního',
				'odpoledne',
				'se',
				'Tomáš',
				'rozhodl',
				'vyrazit',
				'na',
				'malý',
				'ostrov',
				'uprostřed'
			],
			[
				'rybníka.',
				'Už',
				'dlouho',
				'si',
				'říkal,',
				'že',
				'se',
				'tam',
				'musí',
				'podívat,',
				'protože',
				'o něm',
				'kolovalo',
				'spoustu'
			],
			[
				'příběhů.',
				'Lidé',
				'říkali,',
				'že',
				'na',
				'ostrově',
				'roste',
				'starý',
				'strom,',
				'pod',
				'kterým',
				'je',
				'ukrytý',
				'poklad.'
			],
			[
				'Jakmile',
				'však',
				'přistál,',
				'prozkoumal',
				'celý',
				'ostrov,',
				'ale',
				'žádný',
				'poklad',
				'pod',
				'stromem',
				'nenašel.'
			]
		],
		evaluationSegment: [
			{
				range: [
					[0, 0],
					[1, 0]
				], // First sentence
				id: 'fixw-2-0'
			},
			{
				range: [
					[1, 1],
					[2, 1]
				], // Second sentence
				id: 'fixw-2-1'
			},
			{
				range: [
					[2, 2],
					[2, 13]
				], // Third sentence
				id: 'fixw-2-2'
			},
			{
				range: [
					[3, 0],
					[3, 11]
				], // Fourth sentence
				id: 'fixw-2-3'
			}
		]
	}
];

export const pairedReadingLessons: LessonConfigMap['pairedReading']['data'][] = [
	{
		instructionAudioPath: '/sound/tasks/instructions/paired-reading/level1.m4a',
		level: 'level-1',
		label: 'Level 1',
		content: level1content,
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		instructionAudioPath: '/sound/tasks/instructions/paired-reading/level2.m4a',
		level: 'level-2',
		label: 'Level 2',
		content: [
			{
				text: [['auto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					}
				]
			},
			{
				text: [['žena']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					}
				]
			},
			{
				text: [['léto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					}
				]
			},
			{
				text: [['husa']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					}
				]
			},
			{
				text: [['zima']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					}
				]
			},
			{
				text: [['kůra']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					}
				]
			},
			{
				text: [['slunce']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					}
				]
			},
			{
				text: [['příklad']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-7-0-0'
					}
				]
			},
			{
				text: [['dveře']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-8-0-0'
					}
				]
			},
			{
				text: [['město']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-9-0-0'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		instructionAudioPath: '/sound/tasks/instructions/paired-reading/level3.m4a',
		level: 'level-3',
		label: 'Level 3',
		content: [
			{
				text: [['Máma', 'mele', 'maso']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-0-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-0-0-2'
					}
				]
			},
			{
				text: [['Malý', 'chlapec', 'Adam', 'pozoroval', 'veverku.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-1-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-1-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-1-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-1-0-4'
					}
				]
			},
			{
				text: [['Chlupatá', 'veverka', 'držela', 'velký', 'oříšek.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-2-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-2-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-2-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-2-0-4'
					}
				]
			},
			{
				text: [['Veverka', 'oříšek', 'rychle', 'pustila', 'a utekla.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-3-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-3-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-3-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-3-0-4'
					}
				]
			},
			{
				text: [
					[
						'Malý',
						'Pepík',
						'včera',
						'nemohl',
						'se svými',
						'kamarády',
						'skotačit',
						'venku',
						'kvůli',
						'zlomené',
						'noze.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-4-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-4-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-4-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-4-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-4-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-4-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-4-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-4-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-4-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-4-0-10'
					}
				]
			},
			{
				text: [
					[
						'Pepík',
						'si proto',
						'četl',
						'knížky',
						'plné',
						'dobrodružných',
						'příběhů',
						's indiány',
						'nebo',
						'zlotřilými',
						'piráty.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-5-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-5-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-5-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-5-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-5-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-5-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-5-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-5-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-5-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-5-0-10'
					}
				]
			},
			{
				text: [
					[
						'V knihách',
						'se Pepík',
						'pokaždé',
						'nechal',
						'pohltit',
						'odvážnými',
						'příběhy',
						'indiánských',
						'hrdinů.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-6-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-6-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-6-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-6-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-6-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-6-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-6-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-6-0-8'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false
		}
	},
	{
		level: 'level-4',
		label: 'Level 4',
		content: level4content,
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		instructionAudioPath: '/sound/tasks/instructions/paired-reading/level5.m4a',
		level: 'level-5',
		label: 'Level 5',
		content: [
			{
				text: [
					['Můj', 'kamarád', 'Honzík', 'má', 'doma', 'papoušky.', 'Když'],
					['Honzík', 'přijde', 'ze školy,', 'papoušci', 'vesele', 'zapískají.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-0-0'
					},
					{
						range: [
							[0, 6],
							[1, 5]
						], // Second sentence
						id: 'fixw-0-1'
					}
				]
			},
			{
				text: [
					['Malý', 'osmiletý', 'chlapec', 'Adam', 'stál', 'u okna.', 'Sledoval', 'totiž'],
					['silnici', 'vedoucí', 'k jejich', 'domu.', 'Za chvíli', 'by se', 'mělo', 'objevit'],
					['tatínkovo', 'auto', 's přívěsem', 'na koně.', 'Adam', 'má', 'koně', 'moc', 'rád.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-1-0'
					},
					{
						range: [
							[0, 6],
							[1, 3]
						], // Second sentence
						id: 'fixw-1-1'
					},
					{
						range: [
							[1, 4],
							[2, 3]
						], // Third sentence
						id: 'fixw-1-2'
					},
					{
						range: [
							[2, 4],
							[2, 8]
						], // Fourth sentence
						id: 'fixw-1-3'
					}
				]
			},
			{
				text: [
					[
						'Jednoho',
						'odpoledne',
						'se',
						'Kuba',
						'rozhodl,',
						'že',
						'se',
						'vydá',
						'na',
						'výlet',
						'do',
						'lesa',
						'za',
						'vesnicí.'
					],
					[
						'Vzal',
						'si',
						'batůžek,',
						'dal',
						'si',
						'do',
						'něj',
						'svačinu',
						'a vyrazil.',
						'Když',
						'vešel',
						'do',
						'lesa,',
						'slyšel',
						'zpěv'
					],
					[
						'ptáků',
						'a',
						'šumění',
						'stromů.',
						'Šel,',
						'po',
						'úzké',
						'pěšince,',
						'až',
						'narazil',
						'na',
						'starý',
						'dub.',
						'Pod',
						'ním'
					],
					[
						'ležela',
						'malá',
						'krabička.',
						'Kuba',
						'ji',
						'opatrně',
						'otevřel',
						'a',
						'uvnitř',
						'našel',
						'starodávnou',
						'mapu!'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 13]
						],
						id: 'fixw-2-0'
					},
					{
						range: [
							[1, 0],
							[1, 8]
						],
						id: 'fixw-2-1'
					},
					{
						range: [
							[1, 9],
							[2, 12]
						],
						id: 'fixw-2-2'
					},
					{
						range: [
							[2, 13],
							[3, 2]
						],
						id: 'fixw-2-3'
					},
					{
						range: [
							[3, 3],
							[3, 11]
						],
						id: 'fixw-2-4'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false
		}
	},
	{
		instructionAudioPath: '/sound/tasks/instructions/paired-reading/level6.m4a',
		level: 'level-6',
		label: 'Level 6',
		content: level6content,
		partialProps: {
			bufferSize: 150,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	}
];

export const pairedReadingLessons2: LessonConfigMap['pairedReading']['data'][] = [
	{
		level: 'level-1-errors',
		label: 'Level 1 (Chyby)',
		content: level1content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true,
			shouldHighlight: false
		}
	},
	{
		level: 'level-1-noerrors',
		label: 'Level 1 (Bez chyb)',
		content: level1content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		level: 'level-2-errors',
		label: 'Level 2 (Chyby)',
		content: [
			{
				text: [['auto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					}
				]
			},
			{
				text: [['žena']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					}
				]
			},
			{
				text: [['léto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					}
				]
			},
			{
				text: [['husa']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					}
				]
			},
			{
				text: [['zima']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					}
				]
			},
			{
				text: [['kůra']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					}
				]
			},
			{
				text: [['slunce']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					}
				]
			},
			{
				text: [['příklad']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-7-0-0'
					}
				]
			},
			{
				text: [['dveře']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-8-0-0'
					}
				]
			},
			{
				text: [['město']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-9-0-0'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true,
			shouldHighlight: false
		}
	},
	{
		level: 'level-2-noerrors',
		label: 'Level 2 (Bez chyb)',
		content: [
			{
				text: [['auto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					}
				]
			},
			{
				text: [['žena']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					}
				]
			},
			{
				text: [['léto']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					}
				]
			},
			{
				text: [['husa']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					}
				]
			},
			{
				text: [['zima']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					}
				]
			},
			{
				text: [['kůra']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					}
				]
			},
			{
				text: [['slunce']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					}
				]
			},
			{
				text: [['příklad']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-7-0-0'
					}
				]
			},
			{
				text: [['dveře']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-8-0-0'
					}
				]
			},
			{
				text: [['město']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-9-0-0'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		level: 'level-3-errors',
		label: 'Level 3 (Chyby)',
		content: [
			{
				text: [['Máma', 'mele', 'maso']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-0-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-0-0-2'
					}
				]
			},
			{
				text: [['Malý', 'chlapec', 'Adam', 'pozoroval', 'veverku.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-1-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-1-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-1-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-1-0-4'
					}
				]
			},
			{
				text: [['Chlupatá', 'veverka', 'držela', 'velký', 'oříšek.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-2-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-2-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-2-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-2-0-4'
					}
				]
			},
			{
				text: [['Veverka', 'oříšek', 'rychle', 'pustila', 'a utekla.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-3-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-3-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-3-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-3-0-4'
					}
				]
			},
			{
				text: [
					[
						'Malý',
						'Pepík',
						'včera',
						'nemohl',
						'se svými',
						'kamarády',
						'skotačit',
						'venku',
						'kvůli',
						'zlomené',
						'noze.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-4-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-4-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-4-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-4-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-4-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-4-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-4-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-4-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-4-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-4-0-10'
					}
				]
			},
			{
				text: [
					[
						'Pepík',
						'si proto',
						'četl',
						'knížky',
						'plné',
						'dobrodružných',
						'příběhů',
						's indiány',
						'nebo',
						'zlotřilými',
						'piráty.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-5-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-5-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-5-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-5-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-5-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-5-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-5-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-5-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-5-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-5-0-10'
					}
				]
			},
			{
				text: [
					[
						'V knihách',
						'se Pepík',
						'pokaždé',
						'nechal',
						'pohltit',
						'odvážnými',
						'příběhy',
						'indiánských',
						'hrdinů.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-6-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-6-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-6-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-6-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-6-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-6-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-6-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-6-0-8'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true
		}
	},
	{
		level: 'level-3-noerrors',
		label: 'Level 3 (Bez chyb)',
		content: [
			{
				text: [['Máma', 'mele', 'maso']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-0-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-0-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-0-0-2'
					}
				]
			},
			{
				text: [['Malý', 'chlapec', 'Adam', 'pozoroval', 'veverku.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-1-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-1-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-1-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-1-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-1-0-4'
					}
				]
			},
			{
				text: [['Chlupatá', 'veverka', 'držela', 'velký', 'oříšek.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-2-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-2-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-2-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-2-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-2-0-4'
					}
				]
			},
			{
				text: [['Veverka', 'oříšek', 'rychle', 'pustila', 'a utekla.']],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-3-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-3-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-3-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-3-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-3-0-4'
					}
				]
			},
			{
				text: [
					[
						'Malý',
						'Pepík',
						'včera',
						'nemohl',
						'se svými',
						'kamarády',
						'skotačit',
						'venku',
						'kvůli',
						'zlomené',
						'noze.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-4-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-4-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-4-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-4-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-4-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-4-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-4-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-4-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-4-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-4-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-4-0-10'
					}
				]
			},
			{
				text: [
					[
						'Pepík',
						'si proto',
						'četl',
						'knížky',
						'plné',
						'dobrodružných',
						'příběhů',
						's indiány',
						'nebo',
						'zlotřilými',
						'piráty.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-5-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-5-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-5-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-5-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-5-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-5-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-5-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-5-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-5-0-8'
					},
					{
						range: [
							[0, 9],
							[0, 9]
						],
						id: 'fixw-5-0-9'
					},
					{
						range: [
							[0, 10],
							[0, 10]
						],
						id: 'fixw-5-0-10'
					}
				]
			},
			{
				text: [
					[
						'V knihách',
						'se Pepík',
						'pokaždé',
						'nechal',
						'pohltit',
						'odvážnými',
						'příběhy',
						'indiánských',
						'hrdinů.'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 0]
						],
						id: 'fixw-6-0-0'
					},
					{
						range: [
							[0, 1],
							[0, 1]
						],
						id: 'fixw-6-0-1'
					},
					{
						range: [
							[0, 2],
							[0, 2]
						],
						id: 'fixw-6-0-2'
					},
					{
						range: [
							[0, 3],
							[0, 3]
						],
						id: 'fixw-6-0-3'
					},
					{
						range: [
							[0, 4],
							[0, 4]
						],
						id: 'fixw-6-0-4'
					},
					{
						range: [
							[0, 5],
							[0, 5]
						],
						id: 'fixw-6-0-5'
					},
					{
						range: [
							[0, 6],
							[0, 6]
						],
						id: 'fixw-6-0-6'
					},
					{
						range: [
							[0, 7],
							[0, 7]
						],
						id: 'fixw-6-0-7'
					},
					{
						range: [
							[0, 8],
							[0, 8]
						],
						id: 'fixw-6-0-8'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false
		}
	},
	{
		level: 'level-4-errors',
		label: 'Level 4 (Chyby)',
		content: level4content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true,
			shouldHighlight: false
		}
	},
	{
		level: 'level-4-noerrors',
		label: 'Level 4 (Bez chyb)',
		content: level4content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	},
	{
		level: 'level-5-errors',
		label: 'Level 5 (Chyby)',
		content: [
			{
				text: [
					['Můj', 'kamarád', 'Honzík', 'má', 'doma', 'papoušky.', 'Když'],
					['Honzík', 'přijde', 'ze školy,', 'papoušci', 'vesele', 'zapískají.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-0-0'
					},
					{
						range: [
							[0, 6],
							[1, 5]
						], // Second sentence
						id: 'fixw-0-1'
					}
				]
			},
			{
				text: [
					['Malý', 'osmiletý', 'chlapec', 'Adam', 'stál', 'u okna.', 'Sledoval', 'totiž'],
					['silnici', 'vedoucí', 'k jejich', 'domu.', 'Za chvíli', 'by se', 'mělo', 'objevit'],
					['tatínkovo', 'auto', 's přívěsem', 'na koně.', 'Adam', 'má', 'koně', 'moc', 'rád.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-1-0'
					},
					{
						range: [
							[0, 6],
							[1, 3]
						], // Second sentence
						id: 'fixw-1-1'
					},
					{
						range: [
							[1, 4],
							[2, 3]
						], // Third sentence
						id: 'fixw-1-2'
					},
					{
						range: [
							[2, 4],
							[2, 8]
						], // Fourth sentence
						id: 'fixw-1-3'
					}
				]
			},
			{
				text: [
					[
						'Jednoho',
						'odpoledne',
						'se',
						'Kuba',
						'rozhodl,',
						'že',
						'se',
						'vydá',
						'na',
						'výlet',
						'do',
						'lesa',
						'za',
						'vesnicí.'
					],
					[
						'Vzal',
						'si',
						'batůžek,',
						'dal',
						'si',
						'do',
						'něj',
						'svačinu',
						'a vyrazil.',
						'Když',
						'vešel',
						'do',
						'lesa,',
						'slyšel',
						'zpěv'
					],
					[
						'ptáků',
						'a',
						'šumění',
						'stromů.',
						'Šel,',
						'po',
						'úzké',
						'pěšince,',
						'až',
						'narazil',
						'na',
						'starý',
						'dub.',
						'Pod',
						'ním'
					],
					[
						'ležela',
						'malá',
						'krabička.',
						'Kuba',
						'ji',
						'opatrně',
						'otevřel',
						'a',
						'uvnitř',
						'našel',
						'starodávnou',
						'mapu!'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 13]
						],
						id: 'fixw-2-0'
					},
					{
						range: [
							[1, 0],
							[1, 8]
						],
						id: 'fixw-2-1'
					},
					{
						range: [
							[1, 9],
							[2, 12]
						],
						id: 'fixw-2-2'
					},
					{
						range: [
							[2, 13],
							[3, 2]
						],
						id: 'fixw-2-3'
					},
					{
						range: [
							[3, 3],
							[3, 11]
						],
						id: 'fixw-2-4'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true
		}
	},
	{
		level: 'level-5-noerrors',
		label: 'Level 5 (Bez chyb)',
		content: [
			{
				text: [
					['Můj', 'kamarád', 'Honzík', 'má', 'doma', 'papoušky.', 'Když'],
					['Honzík', 'přijde', 'ze školy,', 'papoušci', 'vesele', 'zapískají.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-0-0'
					},
					{
						range: [
							[0, 6],
							[1, 5]
						], // Second sentence
						id: 'fixw-0-1'
					}
				]
			},
			{
				text: [
					['Malý', 'osmiletý', 'chlapec', 'Adam', 'stál', 'u okna.', 'Sledoval', 'totiž'],
					['silnici', 'vedoucí', 'k jejich', 'domu.', 'Za chvíli', 'by se', 'mělo', 'objevit'],
					['tatínkovo', 'auto', 's přívěsem', 'na koně.', 'Adam', 'má', 'koně', 'moc', 'rád.']
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 5]
						], // First sentence
						id: 'fixw-1-0'
					},
					{
						range: [
							[0, 6],
							[1, 3]
						], // Second sentence
						id: 'fixw-1-1'
					},
					{
						range: [
							[1, 4],
							[2, 3]
						], // Third sentence
						id: 'fixw-1-2'
					},
					{
						range: [
							[2, 4],
							[2, 8]
						], // Fourth sentence
						id: 'fixw-1-3'
					}
				]
			},
			{
				text: [
					[
						'Jednoho',
						'odpoledne',
						'se',
						'Kuba',
						'rozhodl,',
						'že',
						'se',
						'vydá',
						'na',
						'výlet',
						'do',
						'lesa',
						'za',
						'vesnicí.'
					],
					[
						'Vzal',
						'si',
						'batůžek,',
						'dal',
						'si',
						'do',
						'něj',
						'svačinu',
						'a vyrazil.',
						'Když',
						'vešel',
						'do',
						'lesa,',
						'slyšel',
						'zpěv'
					],
					[
						'ptáků',
						'a',
						'šumění',
						'stromů.',
						'Šel,',
						'po',
						'úzké',
						'pěšince,',
						'až',
						'narazil',
						'na',
						'starý',
						'dub.',
						'Pod',
						'ním'
					],
					[
						'ležela',
						'malá',
						'krabička.',
						'Kuba',
						'ji',
						'opatrně',
						'otevřel',
						'a',
						'uvnitř',
						'našel',
						'starodávnou',
						'mapu!'
					]
				],
				evaluationSegment: [
					{
						range: [
							[0, 0],
							[0, 13]
						],
						id: 'fixw-2-0'
					},
					{
						range: [
							[1, 0],
							[1, 8]
						],
						id: 'fixw-2-1'
					},
					{
						range: [
							[1, 9],
							[2, 12]
						],
						id: 'fixw-2-2'
					},
					{
						range: [
							[2, 13],
							[3, 2]
						],
						id: 'fixw-2-3'
					},
					{
						range: [
							[3, 3],
							[3, 11]
						],
						id: 'fixw-2-4'
					}
				]
			}
		],
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false
		}
	},
	{
		level: 'level-6-errors',
		label: 'Level 6 (Chyby)',
		content: level6content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: true,
			shouldHighlight: false
		}
	},
	{
		level: 'level-6-noerrors',
		label: 'Level 6 (Bez chyb)',
		content: level6content,
		partialProps: {
			bufferSize: 50,
			logicType: 'main',
			shouldListenForVoice: false,
			shouldEmitMistake: false,
			shouldHighlight: false
		}
	}
];
