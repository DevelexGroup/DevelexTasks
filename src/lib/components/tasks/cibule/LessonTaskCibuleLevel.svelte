<script lang="ts">
	...
	const registerElement = (element: HTMLElement) => {
		...
		if (element.id == FIXATION_EYE) {
			handleLog(sessionId, 'crossFixation', 'started', 'cibule');
	...
	const handleCorrectSyllableClick = () => {
		...
		handleLog(sessionId, 'line', 'completed', 'cibule');
	};
	...
	const handleIncorrectSyllableClick = () => {
		...
		if (mistakeCount >= MAXIMUM_MISTAKE_COUNT) {
			handleLog(sessionId, 'mistake', 'tooManyMistakes', 'cibule');
	...
	const evaluateDwellEvent = (event: GazeInteractionObjectDwellEvent) => {
		...
		if (target.some((t) => t.id === FIXATION_EYE)) {
			...
			handleLog(sessionId, 'crossFixation', 'completed', 'cibule');
		}
	};
	...
	const processCrossFixation = async () => {
		try {
			...
			handleLog(sessionId, 'crossFixation', 'failed', 'cibule');
			dispatch('lessonFail');
		}
	};
