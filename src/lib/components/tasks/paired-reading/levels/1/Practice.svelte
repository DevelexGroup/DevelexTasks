<script lang="ts">
	...
	import { id, practiceRawData } from '$lib/components/tasks/paired-reading/levels/1';
	...
	const data = levelPreset
		? getLevelData(levelPreset, practiceRawData, formatPairedReadingRawData, excludeTags)
		: null;
</script>

{#if data}
	<PairedReadingLevel {id} {data} isPractice />
{/if}
