<script lang="ts">
	...
	import LessonTaskFonologicLine from './LessonTaskFonologicLine.svelte';

	interface Props {
		/**
		 * The content of the task.
		 */
		content?: FonologicTaskType;
		/**
...
>
	{#if content}
		{#each content as task, rowIndex}
			{@const idCorrectSyllable = `fonologic-assignement_${rowIndex}`}
			{@const idOtherSyllableBase = `fonologic-choice_${rowIndex}_`}
			<LessonTaskFonologicLine
				content={task}
				{registerElement}
				{unregisterElement}
				{syllableGap}
